...
class BaseStockCost:
    ...
    def __init__(self):
        ...
        self.realized_name = ['gx', 'rn', 'dx', 'hg', 'pg', 'qz', 'pt']
        self.realize_name = ['pt_realize', 'hg_realize', 'dx_realize', 'qz_realize', 'pg_realize']
        self.hold_name = ['holding', 'duration']
...
class StockCost(BaseStockCost):
    ...
    def deal_decompose(self):
        ...
        for date in dates:
            ...
            res.append(ret)
        res = pd.concat(res)
        res[self.realized_name + self.realize_name + self.hold_name] = res[
            self.realized_name + self.realize_name + self.hold_name].fillna(0)
        del res['index']
        return res
